...
def load_county_data(county_level_name):
    county_level_df = pd.read_csv(county_level_name, quotechar='"', skipinitialspace=True, header=3, usecols=[2,3,22])
    county_level_df.columns = ['state_id', 'county', 'median_income']
...
create_feature_arr(adv_data_df, county_level_df)
    ...
    Next, county level income values corresponding to the county of each person in the advocatia data is placed into the array.
...
def create_feature_arr(adv_data_df, county_level_df):
    ...
    feature_arr[:,1]=adv_data_df['Sex']
    feature_arr[:,2]=adv_data_df['AgeAsOfNow']
    feature_arr[:,3]=adv_data_df['HouseholdSize']
    return feature_arr
...
def split_arrs(feature_arr, value_arr):
    feature_train_arr, feature_test_arr = np.split(feature_arr,[feature_arr.shape[0]/2 + feature_arr.shape[0]%2])
    value_train_arr, value_test_arr = np.split(value_arr,[value_arr.shape[0]/2 + value_arr.shape[0]%2])
...
def gaussian_naive_bayes(arr_tuple):
    gnb = GaussianNB()
    gnb.fit(arr_tuple[0].astype('int'),arr_tuple[1].astype('int'))
    score = gnb.score(arr_tuple[2].astype('int'),arr_tuple[3].astype('int'))
    print score

def sgd_model(arr_tuple):
    sgd = linear_model.SGDClassifier(loss="hinge",penalty="l2", max_iter = 5000, tol = 1e-3)
